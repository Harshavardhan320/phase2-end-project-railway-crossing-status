...
public class UpdateServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			String query= "update trainlist set name=?, address=?, landmark=?, traintime=?, persioncharge=?, status=? where id=?";
			...
			if(!res) {
				request.getRequestDispatcher("edit.jsp").include(request, response);
				out.println("<br><b style='color:green'>Record Updated</b></center>");
			}else {
				request.getRequestDispatcher("edit.jsp").include(request, response);
				out.println("<br><b style='color:red'>Record Fail To Update</b></center>");
			}
